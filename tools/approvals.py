...
# ---------------------------------------------------------------------------
# Gmail bulk-delete scope probe
# ---------------------------------------------------------------------------

GMAIL_FULL_SCOPE = "https://mail.google.com/"
SCOPE_PROBE_CACHE_TTL_HOURS = 24
SCOPE_PROBE_OK_KEY = "gmail_bulk_delete_scope_ok"
SCOPE_PROBE_CHECKED_AT_KEY = "gmail_bulk_delete_scope_checked_at"


async def probe_gmail_bulk_delete_scope_ok() -> bool:
    """Return True iff the live Gmail OAuth refresh_token has the
    ``https://mail.google.com/`` scope required for
    ``users.messages.batchDelete``. Cached in runtime_state for 24h.

    Why: the morning email digest proposes ``gmail_bulk_delete_messages``,
    but the default setup_google_oauth.py script grants only
    ``gmail.modify`` — which does NOT include batchDelete and returns
    HTTP 403 "insufficient scopes". Gating the proposal on this probe
    avoids a known-broken offer.

    Network-failure policy: return False (don't propose deletion) AND do
    not update the cache, so the next call retries.
    """
    ...
    if cached is not None and checked_raw:
        try:
            ...
            if datetime.now(UTC) - checked < timedelta(
                hours=SCOPE_PROBE_CACHE_TTL_HOURS,
            ):
                return cached == "true"
        except (ValueError, TypeError):
    ...
    try:
        ...
        logger.exception(
            "probe_gmail_bulk_delete_scope_ok: probe failed; "
            "returning False (no cache update)",
        )
        return False

    scopes = set(scopes_str.split())
    ok = GMAIL_FULL_SCOPE in scopes
    db.runtime_set(SCOPE_PROBE_OK_KEY, "true" if ok else "false")
    db.runtime_set(
        SCOPE_PROBE_CHECKED_AT_KEY, datetime.now(UTC).isoformat(),
    )
    return ok
