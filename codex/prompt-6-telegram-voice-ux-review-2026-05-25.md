# Prompt 6 — Telegram + Voice UX Review
...
Hermes Agent and OpenClaw both reinforce the same direction: messaging should be a gateway/channel layer with platform-specific affordances, task progress, permissions, and background work surfaces, while the core agent remains channel-agnostic ([Hermes messaging overview](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/index.md), [Hermes Telegram](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/telegram.md), [Hermes Voice Mode](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/features/voice-mode.md), [Hermes GitHub](https://github.com/NousResearch/hermes-agent), [OpenClaw overview](https://docs.openclaw.ai/), [OpenClaw Telegram](https://docs.openclaw.ai/telegram), [OpenClaw background tasks](https://docs.openclaw.ai/automation/tasks), [OpenClaw GitHub](https://github.com/openclaw/openclaw)).
...
- Telegram command menu is defined centrally in `agents/cockpit.py:26-50` and registered with Telegram in `agents/telegram_bridge.py:3094-3097`.
- Incoming text messages are owner-gated in `agents/telegram_bridge.py:575-585`.
- Daily check-in replies are routed before approval handling in `agents/telegram_bridge.py:586-616`.
- Approval typed confirmations are consumed before a normal agent turn in `agents/telegram_bridge.py:618-626`; the approval module documents `CONFIRM-SEND`, reject phrases, and implicit cancel behavior in `tools/approvals.py:65-76` and `tools/approvals.py:139-229`.
- Photo handling saves incoming images, appends compact events, can route classifier output to reminder/receipt/link flows, and drains the media outbox after successful send in `agents/telegram_bridge.py:714-832`.
- Voice handling saves Telegram `.ogg`, enforces max duration, transcribes via `tools.voice.transcribe_voice`, appends a compact event, and sends compound transcripts through `run_compound_turn` when several tasks are detected in `agents/telegram_bridge.py:835-975`.
- Document handling checks magic bytes, builds ingest blocks for PDFs/images/text/HTML, and rejects unsupported or oversized files in `agents/telegram_bridge.py:1374-1503`.
- Inline keyboards exist for approvals, daily check-in, and reminders in `agents/telegram_bridge.py:2358-2378`.
- Callback queries are owner-gated and routed by namespace in `agents/telegram_bridge.py:2385-2498`.
- `/approvals` and `/proactive` are handled in `agents/telegram_bridge.py:2143-2264`; the reminder/check-in callback surfaces live beside them in `agents/telegram_bridge.py:2414-2460`.
...
- Amazon's Alexa conversation design docs explicitly call out "users over-answer a question": real voice users often provide more than one piece of information in a turn, and the system should accept the extra information and continue gathering only what is still missing rather than forcing one-slot-at-a-time speech ([Alexa dialog design](https://developer.amazon.com/en-US/alexa/alexa-haus/dialogs-for-ac)).
- Google's conversation design guidance separates implicit confirmation, explicit confirmation, and no confirmation; it recommends implicit confirmation most of the time, and explicit confirmation only for hard-to-undo actions or exact text shared on the user's behalf ([Google Assistant confirmations](https://developers.google.com/assistant/conversation-design/confirmations)).
- The DialogUSR paper treats multi-intent dialogue as a splitting and reformulation problem: one utterance may need to become several executable sub-queries with recovered omitted context, which is exactly the shape of a multi-task voice note after STT ([DialogUSR paper](https://arxiv.org/abs/2210.11279)).
- Hermes's official Telegram and Voice Mode docs treat voice as a first-class surface: Telegram voice memos can be auto-transcribed, and voice remains part of the same messaging gateway rather than a separate product ([Hermes Telegram](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/telegram.md), [Hermes Voice Mode](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/features/voice-mode.md)).
- OpenClaw's audio docs make the same move: transcribed audio replaces the inbound body while preserving a transcript for downstream command parsing, and its TTS docs support native Telegram voice messages on the way out ([OpenClaw audio and voice notes](https://docs.openclaw.ai/nodes/audio), [OpenClaw text-to-speech](https://docs.openclaw.ai/tools/tts)).

Design implication: after STT, Hikari should explicitly reflect the parsed task list when a voice note contains several actions, then execute safe steps and ask confirmations only where the cost of misunderstanding is high.

### Notification fatigue and proactive assistant UX

Proactive Hikari should be useful without becoming a second inbox:

- Apple's notification HIG says notification actions should save time, be beneficial in context, and prefer short, clear labels; destructive actions need extra context and alerts should not be confused with notifications ([Apple Notifications](https://developer.apple.com/design/human-interface-guidelines/notifications), [Apple Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts)).
- Microsoft Research's field study found that people often value notifications as passive awareness rather than as a demand to switch tasks, even while also describing them as disruptive ([Notifications and Awareness](https://www.microsoft.com/en-us/research/publication/notifications-and-awareness-a-field-study-of-alert-usage-and-preferences/)).
- Horvitz and Apacible argue that proactive systems should reason about the cost of interruption from context signals rather than treating every alert as equally interruptible ([Learning and Reasoning about Interruption](https://www.erichorvitz.com/learninterrupt.htm)).
- Microsoft's HAX Toolkit turns that into direct product guidance for AI systems: proactively initiated interactions should consider the user's task and environment, and may need to postpone action when the user is busy or attention is elsewhere ([Microsoft HAX Guideline 3](https://www.microsoft.com/en-us/haxtoolkit/guideline/time-services-based-on-context/)).
...
- Apple's alerts guidance says alerts interrupt the current task and should be used sparingly for critical information, destructive consequences, purchases, or other important actions; destructive actions should have a clear cancel path ([Apple Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts)).
- Google's conversation design docs recommend explicit confirmation before difficult-to-undo actions and before sending exact text on the user's behalf, while leaving simpler or high-confidence parameters at implicit confirmation or no confirmation ([Google Assistant confirmations](https://developers.google.com/assistant/conversation-design/confirmations)).
- Amazon's dialog-model docs say slot and intent confirmations should be used sparingly because frequent "are you sure" prompts annoy users, but intent confirmation is common for actions like ordering or making reservations, where the full action should be repeated back before execution ([Alexa confirm required information](https://www.developer.amazon.com/it-IT/docs/alexa/custom-skills/define-the-dialog-to-collect-and-confirm-required-information.html)).
- Apple's notification-action guidance also prefers short, beneficial, nondestructive actions in notification surfaces and says destructive actions need enough context to avoid unintended consequences ([Apple Notifications](https://developer.apple.com/design/human-interface-guidelines/notifications)).
...
- Hermes treats messaging as a single gateway process that connects many platforms, handles sessions, runs cron jobs, and delivers voice messages from the same background runtime ([Hermes messaging overview](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/index.md)).
- Hermes's platform table explicitly tracks per-channel capabilities such as voice, images, files, threads, reactions, typing, and streaming; Telegram is shown as supporting voice, images, files, threads, typing, and streaming ([Hermes messaging overview](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/index.md)).
- Hermes's Telegram docs make voice memos a normal part of the chat surface: incoming Telegram voice is auto-transcribed, outgoing TTS is delivered as native voice bubbles, and a local Bot API server is the path for larger media workflows ([Hermes Telegram](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/user-guide/messaging/telegram.md)).
- Hermes's slash-command docs define a cross-platform control surface for reset, status, stop, model, personality, retry, undo, usage, and related actions across Telegram, Discord, Slack, WhatsApp, Signal, and more ([Hermes slash commands](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/reference/slash-commands.md)).
- The GitHub README frames CLI and messaging platforms as parallel entrypoints with one gateway and shared capabilities ([Hermes GitHub](https://github.com/NousResearch/hermes-agent)).
...
- OpenClaw describes itself as a self-hosted gateway across many chat apps, with the Gateway as the source of truth for sessions, routing, and channel connections ([OpenClaw overview](https://docs.openclaw.ai/)).
- Its Telegram docs default DMs to pairing, keep group behavior explicit, and document `requireMention`-style gating for groups; the docs position Telegram as the fastest channel to connect from a phone ([OpenClaw Telegram](https://docs.openclaw.ai/telegram)).
- Its session docs explicitly separate DM, group, room, and cron routing and recommend `per-channel-peer` isolation when multiple people can message the same agent ([OpenClaw sessions](https://docs.openclaw.ai/sessions)).
- Its audio docs replace the inbound body with an `[Audio]` block plus transcript when transcription succeeds, and its TTS docs can deliver native Telegram voice messages on the way out ([OpenClaw audio and voice notes](https://docs.openclaw.ai/nodes/audio), [OpenClaw text-to-speech](https://docs.openclaw.ai/tools/tts)).
- Its background-task docs separate interactive chat from detached work, track `queued -> running -> terminal`, and make completion push-driven rather than polling-driven ([OpenClaw background tasks](https://docs.openclaw.ai/automation/tasks)).
...
- Do not put a high-risk `Confirm` button beside `Reject`. The safer pattern from Apple, Google, and Alexa is to keep destructive actions rare, explicit, fully previewed, and confirmable only when the user understands the exact consequence ([Apple Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts), [Google Assistant confirmations](https://developers.google.com/assistant/conversation-design/confirmations), [Alexa confirm required information](https://www.developer.amazon.com/it-IT/docs/alexa/custom-skills/define-the-dialog-to-collect-and-confirm-required-information.html)).
...
This matches platform guidance that notifications should be timely and high-value, and research arguing that interruption timing should depend on context rather than a flat send-now policy ([Apple Notifications](https://developer.apple.com/design/human-interface-guidelines/notifications), [Notifications and Awareness](https://www.microsoft.com/en-us/research/publication/notifications-and-awareness-a-field-study-of-alert-usage-and-preferences/), [Learning and Reasoning about Interruption](https://www.erichorvitz.com/learninterrupt.htm), [Microsoft HAX Guideline 3](https://www.microsoft.com/en-us/haxtoolkit/guideline/time-services-based-on-context/)).
