...
# Error-message fragments that indicate ON-DISK kuzu corruption. Deliberately
# excludes the same-process lock conflict ("Database path cannot be a directory")
# which is transient and handled by the teardown-and-retry path, NOT a corruption.
_CORRUPTION_SIGNATURES: tuple[str, ...] = (
    "corrupt", "checksum", "malformed", "io error", "ioexception",
    "failed to read", "unexpected end of file", "bad magic", "not a valid database",
)


def _looks_like_corruption(exc: Exception) -> bool:
    """True only for a CLEAR on-disk corruption signature — never the lock/
    directory conflict that the teardown path already recovers from."""
    msg = str(exc).lower()
    if "cannot be a directory" in msg or "lock" in msg:
        return False
    return any(sig in msg for sig in _CORRUPTION_SIGNATURES)


def _quarantine_corrupt_graph(path: Path) -> Path | None:
    """Rename a corrupt kuzu DB (and its WAL) aside so the next open creates a
    fresh one. The graph is derivable — SQLite is the source of truth — so it
    repopulates as new episodes are added. Returns the quarantine path or None."""
    import time as _time
    if not path.exists():
        return None
    dest = path.with_name(f"{path.name}.corrupt.{int(_time.time())}")
    try:
        path.rename(dest)
        wal = path.with_name(path.name + ".wal")
        if wal.exists():
            try:
                wal.rename(path.with_name(dest.name + ".wal"))
            except OSError:
                logger.warning("graph: could not move kuzu WAL aside (non-fatal)")
        logger.critical(
            "graph: quarantined corrupt kuzu db -> %s; rebuilding empty from SQLite truth",
            dest,
        )
        return dest
    except OSError:
        logger.exception("graph: failed to quarantine corrupt kuzu db at %s", path)
        return None
        ...
        while True:
            try:
                ...
                if driver is not None:
                    ...
                    del driver
                    driver = None
                # On a CLEAR on-disk corruption signature (not the transient lock
                # error handled above), quarantine the file and rebuild empty — once.
                if not quarantined_once and _looks_like_corruption(_open_exc):
                    quarantined_once = True
                    _quarantine_corrupt_graph(graph_path)
                    continue
                raise
