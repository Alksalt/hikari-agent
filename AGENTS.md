...
- **jobhunt_radar** — `jobhunt_radar()`. Read-only pipeline snapshot: outreach touches due/overdue, application deadlines, upcoming interviews across outreach.db + job_search.db. Never writes either db.
- **jobhunt_org** — `jobhunt_org(name='DNB')`. Raw outreach.db row for one org by name/fragment; an ambiguous match returns the candidate name list instead of a row.
- **jobhunt_prep** — `jobhunt_prep(slug='dnb-radical-ai')`. Company dossier/positioning/interview-plan/confirmed-stories for one get_hired_prep company folder.
- **jobhunt_draft_touch** — `jobhunt_draft_touch(org='DNB', touch='1')`. Composes + lint-gates a bokmål outreach follow-up, creates a Gmail DRAFT only — never sends, and never writes outreach.db/job_search.db/Notion.
...
- **character-voice** — deeper voice/disclosure grammar, lore, action-line vocabulary. Load `VOICE_DEPTH.md` for heavy beats / disclosures / repair, `LORE_CORE.md` for concrete character facts to weave in.
